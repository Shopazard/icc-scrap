...
class HTMLConverter(PDFConverter):
    ...
    def place_rect(self, color, borderwidth, x, y, w, h):
        ...
        #     self.write('<span style="border: %s %dpx solid; '
    ...
    def begin_div(self, color, borderwidth, x, y, w, h, writing_mode=False):
        ...
        self._font = None
        side = 'left' if (x*self.scale) < 200 else 'center' if (x*self.scale) <400 else 'right'
        self.write(f'<p style="width:100%; height:fit-content; margin:6px; text-align:{side};">')
        return
    ...
    def receive_layout(self, ltpage):
        ...
        def render(item):
            if isinstance(item, LTPage):
                ...
                if self.showpageno:
                    self.write('<div style="display:flex; flex-direction:column;">')
                for child in item:
                    render(child)
